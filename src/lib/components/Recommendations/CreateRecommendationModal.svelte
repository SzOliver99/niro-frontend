<script>
	...
	import { createRecommendationMutation } from '$lib/scripts/queries/recommendation';

	let { selected_user = $bindable(), recommendationCreateModalStore = $bindable() } = $props();

	const userInfoQuery = createQuery(getUserInfoQuery(page.data.token));
	const createRecommendation = createRecommendationMutation(page.data.token);
	...
	async function handleSubmit() {
		...
		$createRecommendation.mutate(
			{ user_uuid: selected_user, payload },
			{
				onSuccess: (data) => {
					Notification.success(data, 3);
					recommendationCreateModalStore.close();
				}
			}
		);
	}
...
{#if $recommendationCreateModalStore}
	<div
	...
	>
		<button
			...
			onclick={recommendationCreateModalStore.close}
		...
		>
			<h1 class="my-4 text-4xl font-semibold text-shadow-md">Ajánlás létrehozása</h1>
